...
static void input_file(const char *path) {
    FILE *f = fopen(path, "r"); if (!f) { perror("fopen"); exit(EXIT_FAILURE); }
    fscanf(f, "%d %d", &n, &m);
    for (int i = 0; i < n; i++) fscanf(f, "%d", &course_duration[i]);
    for (int j = 0; j < m; j++) fscanf(f, "%d", &room_capacity[j]);
    fscanf(f, "%d", &k);
    for (int i = 0; i < n; i++) for (int w = 0; w < w_words; w++) conflict_bits[i][w] = 0ULL;
    for (int e = 0; e < k; e++) {
    ...
    for (int i = 0; i < n; i++) {
        int cnt = 0;
        for (int j = 0; j < m; j++) {
            if (room_capacity[j] >= course_duration[i]) rooms_list[i][cnt++] = j;
        }
        rooms_count[i] = cnt;
    }
    fclose(f);
...
static void input_manual(void) {
    ...
    scanf("%d", &k);
    for (int i = 0; i < n; i++) deg_list[i] = 0;
    for (int i = 0; i < n; i++) for (int w = 0; w < w_words; w++) conflict_bits[i][w] = 0ULL;
    for (int e = 0; e < k; e++) {
...
static inline void dsatur(void) {
    for (int r = 0; r < m; r++) for (int s = 0; s < max_slot; s++) used_room[r][s] = false;
    for (int i = 0; i < n; i++) {
        color_slot[i] = -1;
        sat_deg[i] = 0;
        for (int s = 0; s < max_slot; s++) neighbor_color[i][s] = false;
    }
    int start = 0;
    for (int i = 1; i < n; i++) if (deg_list[i] > deg_list[start]) start = i;
    color_slot[start] = 0;
    for (int v = 0; v < n; v++) {
    ...
    for (int step = 1; step < n; step++) {
        int u = -1;
        for (int i = 0; i < n; i++) if (color_slot[i] < 0) {
            if (u < 0 || sat_deg[i] > sat_deg[u]
                || (sat_deg[i] == sat_deg[u] && deg_list[i] > deg_list[u])) {
                u = i;
            }
        }
        int cmin = 0;
        ...
        color_slot[u] = cmin;
        for (int v = 0; v < n; v++) {
            if (color_slot[v] < 0 && ((conflict_bits[u][v>>6] >> (v & 63)) & 1ULL)) {
                if (!neighbor_color[v][cmin]) {
    ...
    for (int i = 0; i < n; i++) {
        int slot = color_slot[i];
        sol_room[i] = -1;
        for (int idx = 0; idx < rooms_count[i]; idx++) {
            int r = rooms_list[i][idx];
            if (!used_room[r][slot]) {
                used_room[r][slot] = true;
                sol_room[i] = r;
                break;
            }
        }
        if (sol_room[i] < 0 && rooms_count[i] > 0) {
            sol_room[i] = rooms_list[i][0];
        }
    }
}
...
int main(int argc, char **argv) {
    ...
    dsatur();
    for (int i = 0; i < n; i++) {
        printf("%d %d %d\n", i+1, color_slot[i]+1, sol_room[i]+1);
    }
    return 0;
}
